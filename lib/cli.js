...
module.exports = {
    ...
    login: {
        options: {
            ...
            password: {
                help: "ContainerShip Cloud password (leave blank to prompt)",
                metavar: "PASSWORD",
                abbr: "p"
            },
            mfa: {
        ...
        init: function(options){
            var authenticate = function(){
                var request_options = {
                    url: "https://api.containership.io/v1/account/email/token",
                    method: "POST",
                    json: {
                ...
                request(request_options, function(err, response){
                   if(err || response.statusCode != 201){
                    ...
                    config.config.headers["x-containership-cloud-email"] = options.username;
                    config.config.headers["x-containership-cloud-token"] = response.body.token;
    ...
    logout: {
        ...
        init: function(options){
            config.load();
            delete config.config.headers["x-containership-cloud-email"];
            delete config.config.headers["x-containership-cloud-token"];
    ...
    "list-orgs": {
        ...
        init: function(options){
            ...
            var request_options = {
                ...
                method: "GET",
                json: true,
                headers: _.pick(config.config.headers, ["x-containership-cloud-email", "x-containership-cloud-token"])
            }
