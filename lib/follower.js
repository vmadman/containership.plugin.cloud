...
module.exports = {
    ...
    initialize: function(core, config) {
        ...
        core.scheduler.follower.container.add_pre_start_middleware('docker', 'containership-cloud', function(container_options, fn) {
            if(_.has(container_options.env_vars, 'CSC_BACKUP_ID') && !_.isEmpty(container_options.volumes)) {
                ...
                async.forEachOf(container_options.tags.metadata.codexd.volumes, (volume_path, volume_id, fn) => {
                    let options = {
                        url: `https://api.containership.io/v2/organizations${config.organization}/backups/${container_options.env_vars.CSC_BACKUP_ID}/containers/${container_options.id}/volumes/${volume_id}`,
                        method: 'GET',
                        headers: {
                            Authorization: ['Bearer', config.api_key].join(' ')
                        }
        ...
        core.cluster.legiond.on('containership-cloud.backup', function(message) {
            let options = {
                url: `https://api.containership.io/v2/organizations/${config.organization}/backups/${message.data.backup_id}/containers/${message.data.container_id}/volumes/${message.data.volume_id}`,
                method: 'POST',
                headers: {
                    Authorization: ['Bearer', config.api_key].join(' ')
                }
