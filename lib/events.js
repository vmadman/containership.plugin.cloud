...
var send_event = function(json){
    var options = {
        url: ["http://api.containership.io", "v1", process.env.CSC_ORGANIZATION, "events"].join("/"),
        method: "POST",
        timeout: 5000,
        headers: {
    ...
    listen: function(core){
        ...
        core.api.server.server.on("/:api_version/applications/:application", function(route){
            if(route.req.method == "POST" && route.res.stash.code == 201){
                send_event({
                    event: ["Created application", route.req.params.application].join(" ")),
                    cluster_id: core.cluster_id
                });
            }
                ...
                send_event({
                    event: ["Updated application", route.req.params.application].join(" ")),
                    cluster_id: core.cluster_id
                });
            }
                ...
                send_event({
                    event: ["Removed application", route.req.params.application].join(" ")),
                    cluster_id: core.cluster_id
                });
            }
        });
        ...
        core.api.server.server.on("/:api_version/applications/:application/containers", function(route){
            if(route.req.method == "POST" && route.res.stash.code == 201){
                send_event({
                    event: ["Scaled", route.req.params.application, "up by", route.req.query.count, "containers"].join(" ")),
                    cluster_id: core.cluster_id
                });
            }
                ...
                send_event({
                    event: ["Scaled", route.req.params.application, "down by", route.req.query.count, "containers"].join(" ")),
                    cluster_id: core.cluster_id
                });
            }
        });
        ...
        core.api.server.server.on("/:api_version/applications/:application/containers/:container", function(route){
            if(route.req.method == "DELETE" && route.res.stash.code == 204){
                send_event({
                    event: ["Removed", route.req.params.application, "container", route.req.params.container].join(" ")),
                    cluster_id: core.cluster_id
                });
            }
        });
